...
__global__ void letterbox_top_left_bilinear_normalize_kernel(
    uint8_t* src, int src_line_size, int src_width, int src_height,
    ...
    Norm norm)
{
    ...
    float scale = fminf((float)dst_height / (float)src_height,
                        (float)dst_width / (float)src_width);
    int   nw = (int)((float)src_width * scale);
    int   nh = (int)((float)src_height * scale);
    if (nw <= 0)
        nw = 1;
    if (nh <= 0)
        nh = 1;
    ...
    if (dx >= nw || dy >= nh) {
        ...
        if (src_x <= -1.f || src_x >= src_width || src_y <= -1.f ||
            src_y >= src_height) {
            c0 = c1 = c2 = pad_value;
        } else {
            int y_low = floorf(src_y);
            ...
            uint8_t const_value[] = {pad_value, pad_value, pad_value};
            float   ly = src_y - y_low;
            float   lx = src_x - x_low;
            float   hy = 1.f - ly;
            float   hx = 1.f - lx;
            float   w1 = hy * hx, w2 = hy * lx, w3 = ly * hx, w4 = ly * lx;
            uint8_t *v1 = const_value, *v2 = const_value, *v3 = const_value,
                *v4 = const_value;
            if (y_low >= 0) {
                if (x_low >= 0)
                    v1 = src + y_low * src_line_size + x_low * 3;
    ...
    const Norm& norm, cudaStream_t stream)
{
    dim3 grid((dst_width + 31) / 32, (dst_height + 31) / 32);
    dim3 block(32, 32);
    checkKernel(letterbox_top_left_bilinear_normalize_kernel<<<grid, block, 0,
                                                               stream>>>(
