...
static __device__ float sigmoid_rp(float x)
{
    return 1.0f / (1.0f + expf(-x));
}

/** RP 头：0–7 关键点，8 为 conf logit，9–12 颜色，13.. 类别（与 tasks/auto_aim yolov5 parse 一致） */
static __global__ void decode_kernel_rp(float* predict, int num_bboxes,
                                        int num_classes, int output_cdim,
                                        float confidence_threshold,
                                        float letterbox_scale, float* parray,
                                        int MAX_IMAGE_BOXES)
{
    int position = blockDim.x * blockIdx.x + threadIdx.x;
    if (position >= num_bboxes)
    ...
    float conf = sigmoid_rp(pitem[8]);
    if (conf < confidence_threshold)
        return;

    float* color_scores = pitem + 9;
    float  max_color = color_scores[0];
    int    color_id = 0;
    for (int c = 1; c < 4; ++c) {
        if (color_scores[c] > max_color) {
            max_color = color_scores[c];
            color_id = c;
        }
    }

    float* class_scores = pitem + 13;
    float  max_cls = *class_scores++;
    int    class_id = 0;
    for (int i = 1; i < num_classes; ++i, ++class_scores) {
        if (*class_scores > max_cls) {
            max_cls = *class_scores;
            class_id = i;
        }
    }
    ...
    float landmarks[8];
    for (int k = 0; k < 8; k += 2) {
        landmarks[k] = pitem[k] / letterbox_scale;
        landmarks[k + 1] = pitem[k + 1] / letterbox_scale;
    }
    ...
    for (int k = 2; k < 8; k += 2) {
        if (landmarks[k] < min_x)
            min_x = landmarks[k];
        if (landmarks[k] > max_x)
            max_x = landmarks[k];
        if (landmarks[k + 1] < min_y)
            min_y = landmarks[k + 1];
        if (landmarks[k + 1] > max_y)
            max_y = landmarks[k + 1];
    }
    ...
    *pout_item++ = max_y;
    *pout_item++ = conf;
    *pout_item++ = (float)class_id;
    *pout_item++ = 1.f;
    *pout_item++ = landmarks[0];
    *pout_item++ = landmarks[1];
    *pout_item++ = landmarks[6];
    *pout_item++ = landmarks[7];
    *pout_item++ = landmarks[4];
    *pout_item++ = landmarks[5];
    *pout_item++ = landmarks[2];
    *pout_item++ = landmarks[3];
    *pout_item++ = (float)color_id;
}
                                  ...
                                  float rp_letterbox_scale = 1.0f)
{
    ...
    if (type == Type::V8 || type == Type::V8Seg) {
        ...
        checkKernel(decode_kernel_v5_face<<<grid, block, 0, stream>>>(
            ...
            confidence_threshold, invert_affine_matrix, parray,
            MAX_IMAGE_BOXES));
    } else if (type == Type::RP) {
        checkKernel(decode_kernel_rp<<<grid, block, 0, stream>>>(
            predict, num_bboxes, num_classes, output_cdim,
            confidence_threshold, rp_letterbox_scale, parray,
            MAX_IMAGE_BOXES));
    } else {
...
public:
               ...
               AffineMatrix& affine, void* stream = nullptr)
    {
        ...
        if (type_ == Type::RP) {
            float scale =
                std::min((float)network_input_height_ / (float)image.height,
                         (float)network_input_width_ / (float)image.width);
            if (ibatch < (int)rp_scale_.size())
                rp_scale_[ibatch] = scale;

            uint8_t* gpu_workspace = preprocess_buffer->gpu(size_image);
            uint8_t* image_device = gpu_workspace;
            uint8_t* cpu_workspace = preprocess_buffer->cpu(size_image);
            uint8_t* image_host = cpu_workspace;
            memcpy(image_host, image.bgrptr, size_image);
            checkRuntime(cudaMemcpyAsync(image_device, image_host, size_image,
                                         cudaMemcpyHostToDevice, stream_));
            letterbox_top_left_bilinear_normalize(
                image_device, image.width * 3, image.width, image.height,
                input_device, network_input_width_, network_input_height_, 0,
                normalize_, stream_);
            return;
        }
              ...
              float confidence_threshold, float nms_threshold)
    {
        ...
        has_keyPoint = type == Type::V5Face || type == Type::RP;
        if (has_segment_) {
        ...
        if (type == Type::V5 || type == Type::V3 || type == Type::V7) {
            ...
            normalize_ =
                Norm::alpha_beta(1 / 255.0f, 0.0f, ChannelType::SwapRB);
            int out_c = bbox_head_dims_[2];
            if (out_c < 14) {
                INFO("RP model output dim %d < 14 (need 13+ num_classes)",
                     out_c);
                return false;
            }
            num_classes_ = out_c - 13;
        } else {
                                      ...
                                      void* stream = nullptr) override
    {
        ...
        for (int ib = 0; ib < num_image; ++ib) {
            ...
            float rp_sc =
                (type_ == Type::RP && ib < (int)rp_scale_.size())
                    ? rp_scale_[ib]
                    : 1.0f;
        ...
        for (int ib = 0; ib < num_image; ++ib) {
            ...
            for (int i = 0; i < count; ++i) {
                ...
                for (int j = 1; j < 9; ++j)
                    points.push_back(pbox[6 + j]);
                if (keepflag == 1) {
                    int cid = (type_ == Type::RP) ? (int)pbox[15] : -1;
                    Box result_object_box(pbox[0], pbox[1], pbox[2], pbox[3],
                                          pbox[4], label, points, cid);
